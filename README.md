...
Flat, podman-`run`-style verbs at the root — no noun to namespace under. `--config`/`-c` and `--file`/`-f` are persistent flags on the root command (shared by every subcommand), not repeated per-command.

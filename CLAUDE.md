...
- `main.go` — entry point: builds the root Cobra command, owns the shared `--config`/`-c`, `--file`/`-f`, `--git`, and `--git-ref` persistent flags (bundled into one `rootFlags` struct so `newXCmd` constructors take one pointer instead of a growing param list), and registers all five subcommands directly. If you're new to Go, start here.
...
- `cmd_check.go` — `astrona check`, no `*rootFlags` needed (it doesn't touch a lab config). Verifies every external binary astrona shells out to (`astronaDepChecks`), split into required (the default `kind` runtime's own toolchain: `kind`, `docker`/`podman`, `kubectl`) and optional (qemu toolchain, `git` — only needed for a specific runtime or `--git`). A missing required dependency is a non-zero exit; a missing optional one is a warning only.

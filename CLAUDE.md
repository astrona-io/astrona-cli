...
- **QEMU base images** (`acquireBaseImage` in `hypervisor.go`): unlike scripts/manifests, a VM base image is a full bootable OS — a mandatory `sha256:<hex>` checksum is required for every image (`file`, `url`, or `oci` source alike) and enforced before the VM ever boots off it; a mismatch aborts and, for a downloaded/pulled image, deletes the temp path (never a user's own local image file). Don't make this optional for any source type. The `oci` source (`pullOCIImage`) shells out to `oras pull` — same trust posture already given to `kind`/`docker`/`kubectl`/`qemu-img` as external binaries: astrona doesn't control or bound that download itself, the checksum check afterward is what matters. `url`/`oci` fetches are cached at `~/.astrona/cache/images/<algo>-<hex>.qcow2` (`cacheHit`/`persistToCache`) so a repeat `astrona run` doesn't re-download/re-pull an already-verified image — the checksum is re-verified on every cache hit too (`cacheHit` discards and falls back to a fresh fetch on mismatch), so caching never becomes a second, unverified path onto disk. Keep it that way if you touch this.
